...
parser = argparse.ArgumentParser(description='[*] Reverting ascii strings into Intel 32-bit hex format.')
    ...
    if string.find('/') > -1:
        ...
        for i in range(len(ocurrences_index)):
            if i != len(ocurrences_index)-1:
                ...
                if bytes_size < 4: # if there is not 4 bytes between slashes
...
< The x86 "Little-Hexdian" String Converter >
---------------------------------------------
	\   ^__^
	 \  (oo)\_______
	    (__)\       )\/\\
...
input_remainder = len(string) % 4

if input_remainder > 0:
    print("\nStack needs to be aligned!")
    alignment_bytes = (4 - input_remainder) + len(string)
...
if input_remainder > 0:
        ...
        if index  > -1:
            string = string[:index] + (4-input_remainder)*'/' + string[index:]
        else:
            string = string + ((4-input_remainder)*'/')

print("[*] Final String: {}\n".format(string))

print("[*] Converting it ...\n")

stringList= [string[index:index+4] for index in range(0,len(string),4)]

# revert and output string in dword groups with hex values

print("* * * * * * * * * *\n") 
for dword in stringList[::-1]:
  
    print(dword[::-1]+ ' : 0x'+ str(binascii.hexlify(dword[::-1].encode('utf-8')),'ascii'))
